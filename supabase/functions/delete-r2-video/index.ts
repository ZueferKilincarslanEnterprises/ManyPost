...
serve(async (req: Request) => {
  ...
  try {
    // [delete-r2-video] Initialize Supabase client
    const supabaseUrl = Deno.env.get('SUPABASE_URL')!;
    const supabaseServiceKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!;
    const supabase = createClient(supabaseUrl, supabaseServiceKey);

    // [delete-r2-video] Authenticate using service key (since this is called from backend)
    ...
    if (token !== supabaseServiceKey) {
      console.error('[delete-r2-video] Invalid service key');
    ...
    if (!r2Key) {
      console.error('[delete-r2-video] Missing r2Key');
