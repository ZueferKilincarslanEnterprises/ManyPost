...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    for (const post of posts) {
      try {
        // Access Token für den spezifischen User holen
        ...
        if (!integration?.access_token) {
          console.warn(`[sync-video-stats] Kein Token für User ${post.user_id} gefunden.`);
          continue;
        }
        ...
        if (stats) {
          await supabase.from('video_stats').insert({
            ...
            comment_count: parseInt(stats.comment_count || '0')
          });
          successCount++;
        }
      } catch (err) {
